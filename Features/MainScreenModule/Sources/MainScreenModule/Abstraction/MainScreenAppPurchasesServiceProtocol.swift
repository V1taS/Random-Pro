...
public protocol MainScreenAppPurchasesServiceProtocol {
  
  /// Получить доступные продукты
  /// - Parameter completion: Возвращает список продуктов
  func getProductsForMain(completion: @escaping ([MainScreenApphudProductProtocol]?) -> Void)
  
  /// Делаем покупку продукта
  /// - Parameters:
  ///  - product: Продукт для покупки
  ///  - completion: Возвращает статус покупки
  func purchaseWithForMain(_ product: any MainScreenApphudProductProtocol,
...
public protocol MainScreenApphudProductProtocol {
...
public protocol MainScreenAppPurchasesServiceStateProtocol {
