...
public final class HapticServiceImpl: HapticService {
  public init() {}
  ...
  public func play(isRepeat: Bool,
    ...
    createHapticEngine { [weak self] createResult in
      switch createResult {
      case .success:
        self?.getPatternFrom(patternType: patternType) { [weak self] patternResult in
          switch patternResult {
          case let .success(pattern):
            self?.playPattern(pattern, isRepeat: isRepeat) { [weak self] in
...
private extension HapticServiceImpl {
  ...
  func getPatternFrom(patternType: HapticPatternType,
