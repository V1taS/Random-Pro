      ...
      self?.networkService.performRequestWith(
        ...
        case let .success(data):
          DispatchQueue.main.async {
            guard let model = self?.networkService.map(data, to: [FilmsScreenEngModelDTO].self) else {
              self?.output?.somethingWentWrong()
              return
            }
            completion(model)
  ...
  func loadListRusFilmsDto(completion: @escaping ([FilmsScreenRusModelDTO.Film]) -> Void) {
    DispatchQueue.global(qos: .userInteractive).async { [weak self] in
      ...
      self?.networkService.performRequestWith(
        ...
        case let .success(data):
          DispatchQueue.main.async {
            guard let model = self?.networkService.map(data, to: FilmsScreenRusModelDTO.self) else {
              self?.output?.somethingWentWrong()
              return
            }
